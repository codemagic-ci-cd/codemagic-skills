...
name: codemagic-first-build-mobile
...
* a valid artifact is produced:
  ...
  * Android → APK/AAB
  * iOS → IPA (only if explicitly requested; otherwise Android debug is preferred for first validation)
...
* Choose **Android debug build first** unless user explicitly asks for iOS
...
* missing certs/profiles → inform user, optionally switch to Android first
